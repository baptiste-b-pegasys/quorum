...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	if eth.config.QuorumLightClient.Enabled() {
		...
		// TODO:bbo merge check > eth.APIBackend = &EthAPIBackend{stack.Config().ExtRPCEnabled(), stack.Config().AllowUnprotectedTxs, eth, nil}
...
func (s *Ethereum) CalcGasLimit(block *types.Block) uint64 {
	...
	return core.CalcGasLimit(block.GasUsed(), block.GasLimit(), minGasLimit, s.config.Miner.GasFloor, s.config.Miner.GasCeil)
}
