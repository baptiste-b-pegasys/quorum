...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	minGasLimit := w.chainConfig.GetMinerMinGasLimit(parent.Number(), params.DefaultMinGasLimit) // Quorum
	num := parent.Number()
	header := &types.Header{
		ParentHash: parent.Hash(),
		Number:     num.Add(num, common.Big1),
		GasLimit:   core.CalcGasLimit(parent.GasUsed(), parent.GasLimit(), minGasLimit, w.config.GasFloor, w.config.GasCeil), // TODO:bbo merge
		Extra:      w.extra,
